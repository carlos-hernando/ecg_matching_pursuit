function heasig=readheader_nsrdb(name);
% READHEADER function reads the header of the nsrdb (normal sinus database) signal files
...
if (exist('hour','var') && exist('year','var')) heasig.date=datenum(year,month,day,hour,min,sec); end
%Change in this statement: not only the hour is needed, also the date. As
%an example in the snrdb database the hour is available but not the date
